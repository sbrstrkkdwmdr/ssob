...
export class TrackAR extends OsuCommand {
    ...
    async execute() {
        ...
        if (
            ...
            !this.params.user ||
            this.params.user.length < 1
        ) {
            await this.sendError(helper.errors.tracking.nullUser);
        }

        const guildsetting = await helper.vars.guildSettings.findOne({
            where: {
                guildid:
        ...
        if (!guildsetting?.dataValues?.trackChannel) {
            ...
            guildsetting?.dataValues?.trackChannel !=
            (this.input?.message?.channelId ??
                this.input?.interaction?.channelId)
        ) {
            await this.sendError(
                helper.errors.tracking.channel_wrong(
                    guildsetting?.dataValues?.trackChannel,
                ),
            );
        }
