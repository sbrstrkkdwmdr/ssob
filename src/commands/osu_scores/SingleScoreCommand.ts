...
export class SingleScoreCommand extends OsuCommand {
    ...
    async renderEmbed() {
        let rspassinfo = "";
        let totalhits = this.totalHits(
            this.score.statistics,
            this.score.ruleset_id,
        );

        const getHits = formatters.returnHits(
            this.score.statistics,
            this.score.ruleset_id,
        );
        let hitlist: string =
        ...
        const curbmhitobj =
            ...
            this.map.count_spinners;
        let msToFail: number,
            curbmpasstime: number,
            guesspasspercentage: number;
        if (!this.score.passed) {
            msToFail = await other.getFailPoint(
                totalhits,
                `${helper.path.files}/maps/${this.map.id}.osu`,
            );
        ...
        if (!this.score.passed) {
            ...
            rsgrade =
                helper.emojis.grades.F +
                `(${helper.emojis.grades[this.grade().rank.toUpperCase()]} if pass)`;
        }
    ...
    async perf(failed: {
        ...
        try {
            ...
            perfs = await performance.fullPerformance(
                ...
                failed.objectsHit,
                new Date(
                    this.score?.beatmap?.last_updated ?? this.map.last_updated,
                ),
                overrides.cs,
                overrides.ar,
                overrides.od,
            ...
            const mxCombo =
                perfs[0]?.difficulty?.maxCombo ?? this.map?.max_combo;
    ...
    async graph(x: any[], y: number[], point: number = -1) {
        ...
        if (point == -1 || point >= 1) {
            ...
            graph = new LineGraphBuilder({
                ...
                colours: [
                    helper.colours.rainbowPastelRGB.green,
                    helper.colours.rainbowPastel.grey,
                ],
                settings: {
                    isCurved: true,
                    fill: true,
    ...
    setEmbed({
        ...
        let embed = new Discord.EmbedBuilder()
            ...
            .setURL(
                this.score.id
                    ? `https://osu.ppy.sh/scores/${this.score.id}`
                    : `https://osu.ppy.sh/b/${this.map.id}`,
            )
            ...
            .addFields([
                {
                    name: "SCORE DETAILS",
                    value:
                        `${other.formatScore(this.score)} ${scorerank}
...
`);
                formatters.userAuthor(
                    this.osudata,
                    embed,
                    this.params.overrideAuthor,
                );
                break;
            case "recent":
                embed.setTitle(
                    `#${this.params.page + 1} most recent ${this.params.showFails == 1 ? "play" : "pass"} for ${this.score.user.username} | ${formatters.relativeTime(this.score.ended_at)}`,
                )
