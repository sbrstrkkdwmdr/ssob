...
export class MapLeaderboard extends OsuCommand {
    ...
    async execute() {
        ...
        const buttons = new Discord.ActionRowBuilder().addComponents(
            new Discord.ButtonBuilder()
                .setCustomId(
                    `${helper.versions.releaseDate}-Map-${this.name}-any-${this.input.id}-${this.params.mapid}${this.params.mapmods && this.params.mapmods.length > 0 ? "+" + this.params.mapmods.join(",") : ""}`,
                )
    ...
    async getScores() {
        ...
        if (
            data.findFile(this.input.id, "lbdata") &&
    ...
    protected createEmbed(data: formatterInfo<osuapi.v2.types.Score>) {
        const lbEmbed = new Discord.EmbedBuilder()
            ...
            .setTitle(
                `Score leaderboard of \`${this.mapTitle(this.map, this.map.beatmapset)}\``,
            )
