...
sqlite3: [${pkgjson.dependencies["sqlite3"].replace("^", "")}](https://github.com/TryGhost/node-sqlite3)`,
            ...
            preGlobal: helper.vars.config.prefix.includes("`")
                ? `"${helper.vars.config.prefix}"`
                : `\`${helper.vars.config.prefix}\``,
            preServer: serverpfx.includes("`")
                ? `"${serverpfx}"`
                : `\`${serverpfx}\``,
        ...
        if (this.input.args.length > 0) {
            ...
            switch (this.input.args[0]) {
                case "uptime":
                    Embed.setTitle("Total uptime").setDescription(data.uptime);
                    break;
                case "version":
                case "v":
                    Embed.setTitle("Bot version").setDescription(data.version);
                    break;
                case "server":
                    Embed.setTitle("Bot server").setDescription(data.server);
                    break;
                case "website":
                    Embed.setTitle("Bot website").setDescription(data.website);
                    break;
                case "dependencies":
                case "dep":
                case "deps":
                    Embed.setTitle("Dependencies").setDescription(data.deps);
                    break;
                case "source":
                case "code":
                    Embed.setTitle("Source Code").setDescription(data.source);
                    break;
                default:
                    Embed.setDescription(
                        `\`${this.input.args[0]}\` is an invalid argument`,
                    );
                    break;
            }
        } else {
