...
export class Changelog extends Command {
    ...
    async setParamsBtn() {
        ...
        const curpage =
            parseInt(this.input.message.embeds[0].footer.text.split("/")[0]) -
            1;
        ...
        if (
            this.input.message.embeds[0].title
                .toLowerCase()
    ...
    findVersion_exact() {
        this.found = helper.versions.versions.findIndex(
            (x) =>
                this.params.version.trim() === `${x.name}`.trim() ||
                this.params.version.includes(`${x.releaseDate}`) ||
                this.params.version.includes(`${x.releaseDate}`) ||
                `${x.releaseDate}`.includes(this.params.version) ||
                `${x.releaseDate}`.includes(this.params.version),
        );
        this.params.foundBool = true;
        if (this.found == -1) {
    ...
    fixFound() {
        if (
            ((!this.params.foundBool && this.found != "string") ||
                (this.params.page && this.found == "string")) &&
        ...
        if (isNaN(this.params.useNum) || !this.params.useNum)
            this.params.useNum = 0;
        if (
            this.params.useNum + 1 >=
            Math.ceil(helper.versions.versions.length / 10)
        ) {
            this.params.useNum =
                Math.ceil(helper.versions.versions.length / 10) - 1;
        }
    }
    createEmbed() {
        const embed = new Discord.EmbedBuilder();
        const doc = fs.readFileSync(
            `${helper.path.cache}/changelog.md`,
            "utf-8",
        );
    ...
    embedListVersions(input: string[], offset: number) {
        ...
        for (
            let i = 0;
            offset + i < helper.versions.versions.length && i < 10;
            i++
        ) {
    ...
    embedVersion(embed: Discord.EmbedBuilder, input: string[]) {
        ...
        const commitURL =
            /^(https?:\/\/)?([a-zA-Z0-9-]+\.)+[a-zA-Z]{2,}(:[0-9]{1,5})?(\/[^\s]*)?$/g.test(
                commit,
            )
                ? commit
        ...
        embed
            ...
            .setURL(
                "https://github.com/sbrstrkkdwmdr/ssob/blob/dev/changelog.md",
            )
            .setDescription(
                `commit [${
                    commit.includes("commit/")
                        ? commitURL
                              .split("commit/")[1]
                              .trim()
                              ?.slice(0, 7)
...
`,
    ...
    versionChangeStringArray(version: string) {
        const changesTxt = version.includes("</br>")
            ? version.split("</br>")[1]
            : version.split("\n").slice(3).join("\n");
        return changesTxt
            ? changesTxt
    ...
    changeCategory(change: string) {
        ...
        if (clrs[temp]) {
            return colourcalc.codeBlockColourText(
                temp.toUpperCase(),
                clrs[temp],
                "text",
            );
        ...
        if (this.params.isList) {
            buttons.setComponents(
                new Discord.ButtonBuilder()
                    .setCustomId(
                        `${helper.versions.releaseDate}-Detail0-${this.name}-${this.commanduser.id}-${this.input.id}`,
                    )
    ...
    async buttonPages() {
        ...
        this.disablePageButtons_check(
            ...
            false,
            this.params.useNum == 0,
            (this.params.useNum + 1 >= helper.versions.versions.length &&
                !this.params.isList) ||
                (this.params.useNum + 1 >=
                    Math.ceil(helper.versions.versions.length / 10) &&
                    this.params.isList),
