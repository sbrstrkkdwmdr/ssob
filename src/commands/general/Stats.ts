...
export class Stats extends Command {
    ...
    async execute() {
        ...
        // do stuff
        const trueping =
            (this.input.message ?? this.input.interaction).createdAt.getTime() -
            new Date().getTime() +
            "ms";

        const uptime = Math.round(
            (new Date().getTime() - helper.vars.startTime.getTime()) / 1000,
        );
        const uptimehours =
            Math.floor(uptime / 3600) >= 10
                ? Math.floor(uptime / 3600)
                : "0" + Math.floor(uptime / 3600);
        const uptimeminutes =
            Math.floor((uptime % 3600) / 60) >= 10
                ? Math.floor((uptime % 3600) / 60)
                : "0" + Math.floor((uptime % 3600) / 60);
        const uptimeseconds =
            Math.floor(uptime % 60) >= 10
                ? Math.floor(uptime % 60)
                : "0" + Math.floor(uptime % 60);
