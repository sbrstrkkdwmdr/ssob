...
export class CheckPerms extends Command {
    ...
    async setParamsMsg() {
        if (this.input.args[0]) {
            if (this.input.message.mentions.users.size > 0) {
                this.params.searchUser =
                    this.input.message.mentions.users.first();
            } else {
                this.params.searchUser = helper.vars.client.users.cache.get(
    ...
    async execute() {
        ...
        // do stuff
        if (
            this.params.searchUser == null ||
            typeof this.params.searchUser == "undefined"
        ) {
            this.params.searchUser = this.commanduser;
        }

        if (!(
            checks.isAdmin(this.commanduser.id, this.input.message?.guildId) ||
            checks.isOwner(this.commanduser.id)
        )) {
            this.params.searchUser = this.commanduser;
        }
        const embed = new Discord.EmbedBuilder();
        try {
            const userAsMember = this.input.message.guild.members.cache.get(
                this.params.searchUser.id,
            );
            //get perms
            ...
            embed
                .setTitle("Error")
                .setDescription(
                    "An error occured while trying to get the permissions of the user.",
                )
                .setColor(helper.colours.embedColour.admin.dec);
        }
