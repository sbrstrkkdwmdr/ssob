...
export class Clear extends Command {
    ...
    async execute() {
        ...
        this.logInput();
        // do stuff
        let embed = new Discord.EmbedBuilder().setTitle("Clearing cache");
    ...
    clearCache(type: string, embed: Discord.EmbedBuilder) {
        switch (type) {
            ...
            default:
                {
                    //clears all temprary files (cache/commandData)
                    this.clearTemporary();
                    embed.setDescription(
                        `Clearing temporary files in ./cache/commandData/\n(ranked/loved/approved maps are kept)`,
                    );
                }
                break;
            case "all":
                {
                    //clears all files in commandData
                    this.clearCommandData();
                    embed.setDescription(
                        `Clearing all files in ./cache/commandData/`,
                    );
                }
                break;
            case "trueall":
                {
                    //clears everything in cache
                    ...
                    embed.setDescription(
                        `Clearing all files in ./cache/ and ./files/maps`,
                    );
                }
                break;
            case "mapall":
            case "mapsall":
                {
                    // clears all maps and mapset files
                    this.clearMapMetadata();
                    this.clearMapFiles();
                    embed.setDescription(
                        `Clearing all map-related files in ./cache/commandData/ and ./files/maps/`,
                    );
                }
                break;
            case "mapmeta":
                {
                    this.clearMapMetadata();
                    embed.setDescription(
                        `Clearing all map-related files in ./cache/commandData/`,
                    );
                }
                break;
            case "mapobjects":
            ...
            case "users":
                {
                    //clears all osudata files
                    this.clearUserData();
                    embed.setDescription(
                        `Clearing all user data files in ./cache/commandData/`,
                    );
                }
                break;
            case "previous":
                {
                    // clears all previous files
                    this.clearPreviousMaps();
                    this.clearPreviousScores();
                    this.clearPreviousUsers();
                    embed.setDescription(
                        `Clearing all files in ./cache/previous/`,
                    );
                }
                break;
            case "pmaps":
                {
                    // clears all previous map files
                    this.clearPreviousMaps();
                    embed.setDescription(
                        `Clearing all previous map files in ./cache/previous/`,
                    );
                }
                break;
            case "pscores": {
                // clears all previous score files
                this.clearPreviousScores();
                embed.setDescription(
                    `Clearing all previous score files in ./cache/previous/`,
                );
            }
            case "pusers":
                {
                    // clears all previous user files
                    this.clearPreviousUsers();
                    embed.setDescription(
                        `Clearing all previous user files in ./cache/previous/`,
                    );
                }
                break;
            case "errors":
                {
                    ...
                    this.clearErrors();
                    embed.setDescription(
                        `Clearing error files in ./cache/errors/`,
                    );
                }
                break;
            case "graph": {
    ...
    clearTemporary() {
        log.stdout(
            `manually clearing temporary files in ${helper.path.cache}/commandData/`,
        );
    ...
    clearCommandData() {
        log.stdout(
            `manually clearing all files in ${helper.path.cache}/commandData/`,
        );
        const curpath = `${helper.path.cache}/commandData`;
        const files = fs.readdirSync(curpath);
        for (const file of files) {
    ...
    clearMapMetadata() {
        log.stdout(
            `manually clearing all map and mapset files in ${helper.path.cache}/commandData/`,
        );
        const curpath = `${helper.path.cache}/commandData`;
        const files = fs.readdirSync(curpath);
        for (const file of files) {
    ...
    clearMapFiles() {
        log.stdout(
            `manually clearing all map files in ${helper.path.files}/maps/`,
        );
    ...
    clearUserData() {
        log.stdout(
            `manually clearing all osudata files in ${helper.path.cache}/commandData/`,
        );
        const curpath = `${helper.path.cache}/commandData`;
        const files = fs.readdirSync(curpath);
        for (const file of files) {
    ...
    clearPreviousMaps() {
        log.stdout(
            `manually clearing all prevmap files in ${helper.path.cache}/previous/`,
        );
    ...
    clearPreviousScores() {
        log.stdout(
            `manually clearing all prev score files in ${helper.path.cache}/previous/`,
        );
        const curpath = `${helper.path.cache}/previous`;
        const files = fs.readdirSync(curpath);
        for (const file of files) {
    ...
    clearPreviousUsers() {
        log.stdout(
            `manually clearing all prev user files in ${helper.path.cache}/previous/`,
        );
        const curpath = `${helper.path.cache}/previous`;
        const files = fs.readdirSync(curpath);
        for (const file of files) {
    ...
    clearErrors() {
        log.stdout(
            `manually clearing all err files in ${helper.path.cache}/errors/`,
        );
    ...
    clearGraphs() {
        log.stdout(
            `manually clearing all graph files in ${helper.path.cache}/graphs/`,
        );
    ...
    clearParams() {
        log.stdout(
            `manually clearing all param files in ${helper.path.cache}/params/`,
        );
