...
export class Ranking extends OsuCommand {
    ...
    async setParamsMsg() {
        ...
        this.params.country = this.setParam(
            this.params.country,
            ["-country"],
            "string",
            {},
        ...
        this.params.type = this.setParamBoolList(
            this.params.type,
        ...
        this.params.spotlight = this.setParam(
    ...
    async setParamsInteract() {
        ...
        this.params.type = (interaction.options.getString("type") ??
            "performance") as osuapi.types_v2.RankingType;
        this.params.page = interaction.options.getInteger("page") ?? 0;
        this.params.spotlight =
            interaction.options.getInteger("spotlight") ?? undefined;
    }
    async setParamsBtn() {
    ...
    async execute() {
        ...
        if (
            data.findFile(this.input.id, "rankingdata") &&
        ...
        if (this.input.buttonType == null && this.params.type != "country") {
            data.userStatsCache(
                rankingdata.ranking as osuapi.types_v2.UserStatistics[],
        ...
        if (this.params.parse && this.params.type != "country") {
            ...
            this.input.overrides = {
                mode: this.params.mode,
                id: (
                    rankingdata?.ranking[pid] as osuapi.types_v2.UserStatistics
                )?.user.id,
                commanduser: this.commanduser,
                commandAs: this.input.type,
            };
            if (
                this.input.overrides.id == null ||
                typeof this.input.overrides.id == "undefined"
            ) {
                await this.sendError(
                    `${helper.errors.score.nf} at index ${pid}`,
                );
                return;
            }
        ...
        this.params.country != "ALL"
            ? embed.setThumbnail(
                  `https://osuhelper.argflags.omkserver.nl${this.params.country}`,
              )
            : "";
        ...
        if (this.params.type == "country") {
            this.formatCL(
                embed,
                rankingdata.ranking as osuapi.types_v2.CountryStatistics[],
            );
        } else {
            this.formatUL(
                embed,
    ...
    formatCL(
        ...
        rankingdata: osuapi.types_v2.CountryStatistics[],
    ) {
        for (
            let i = 0;
            i < 5 && i + this.params.page * 5 < rankingdata.length;
            i++
        ) {
            ...
            embed.addFields([
                {
                    ...
                    value: `:flag_${country.code.toLowerCase()}:
...
`,
    ...
    formatUL(
        embed: Discord.EmbedBuilder,
        rankingdata: osuapi.types_v2.UserStatistics[],
    ) {
        for (
            let i = 0;
            i < 5 && i + this.params.page * 5 < rankingdata.length;
            ...
            let parseGlobalRank =
                num == user?.global_rank
                    ...
                    : user.global_rank == null
                      ? ""
                      : "(#" +
                        calculate.separateNum(user.global_rank) +
                        " Global)";
            embed.addFields([
                {
                    name: `#${num} ${parseGlobalRank}`,
                    value: `:flag_${user.user.country_code.toLowerCase()}: [${user.user.username}](https://osu.ppy.sh/users/${user.user.id}/${this.params.mode})
