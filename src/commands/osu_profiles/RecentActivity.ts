...
export class RecentActivity extends OsuCommand {
    ...
    async setParamsBtn() {
        ...
        this.params.user = this.input.message.embeds[0].url
            .split("users/")[1]
            .split("/")[0];
        this.params.page = parseInt(
            this.input.message.embeds[0].description
                .split("Page: ")[1]
                .split("/")[0],
        );
    ...
    async execute() {
        ...
        const curEmbed = new Discord.EmbedBuilder()
            ...
            .setURL(
                `https://osu.ppy.sh/users/${osudata.id}/${osudata.playmode}#recent`,
            )
            .setThumbnail(
                `${osudata?.avatar_url ?? helper.defaults.images.any.url}`,
            )
            .setDescription(
                `Page: ${this.params.page + 1}/${Math.ceil(rsactData.length / pageLength)}`,
            );
        ...
        for (let i = 0; i < rsactData.length && i < pageLength; i++) {
            ...
            switch (curEv.type) {
                ...
                case "beatmapsetApprove":
                    {
                        const temp =
                            curEv as osuapi.types_v2.EventBeatmapsetApprove;
                ...
                case "beatmapPlaycount":
                    {
                        const temp =
                            curEv as osuapi.types_v2.EventBeatmapPlaycount;
                        obj.desc = `Achieved ${temp.count} plays on [\`${temp.beatmap.title}\`](https://osu.ppy.sh${temp.beatmap.url}) <t:${new Date(temp.created_at).getTime() / 1000}:R>`;
                    }
                    break;
                case "beatmapsetDelete":
                    {
                        const temp =
                            curEv as osuapi.types_v2.EventBeatmapsetDelete;
                ...
                case "beatmapsetRevive":
                    {
                        const temp =
                            curEv as osuapi.types_v2.EventBeatmapsetRevive;
                ...
                case "beatmapsetUpdate":
                    {
                        const temp =
                            curEv as osuapi.types_v2.EventBeatmapsetUpdate;
                ...
                case "beatmapsetUpload":
                    {
                        const temp =
                            curEv as osuapi.types_v2.EventBeatmapsetUpload;
                ...
                case "rank":
                    {
                        ...
                        obj.desc = `Achieved rank **#${temp.rank}** on [\`${temp.beatmap.title}\`](https://osu.ppy.sh${temp.beatmap.url}) (${helper.emojis.gamemodes[temp.mode]}) <t:${new Date(temp.created_at).getTime() / 1000}:R>`;
                    }
                    break;
                case "rankLost":
                    {
                ...
                case "userSupportAgain":
                    {
                        const temp =
                            curEv as osuapi.types_v2.EventUserSupportAgain;
                ...
                case "userSupportFirst":
                    {
                        const temp =
                            curEv as osuapi.types_v2.EventUserSupportFirst;
                ...
                case "userSupportGift":
                    {
                        const temp =
                            curEv as osuapi.types_v2.EventUserSupportGift;
                ...
                case "usernameChange":
                    {
                        const temp =
                            curEv as osuapi.types_v2.EventUsernameChange;
