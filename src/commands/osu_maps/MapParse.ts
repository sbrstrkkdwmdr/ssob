...
export class MapParse extends OsuCommand {
    declare protected params: {
        ...
        overrideBpm: number;
        overwriteModal:
            Discord.StringSelectMenuComponent | Discord.StringSelectMenuBuilder;
    ...
    async setParamsMsg() {
        ...
        this.params.showBg = this.setParam(
            this.params.showBg,
            ["-bg"],
            "bool",
            {},
        ...
        this.params.query = this.setParam(this.params.query, ["-?"], "string", {
            string_isMultiple: true,
        });

        this.params.isppCalc = this.setParam(
            this.params.isppCalc,
            ["pp", "calc", "performance"],
            "bool",
            {},
        );
        ...
        mapTemp.mode && !this.params.mode
            ? (this.params.mode = mapTemp.mode)
            : null;
    ...
    async setParamsInteract() {
        ...
        this.params.mapmods = osumodcalc.mod.fromString(
            interaction.options.getString("mods").toUpperCase(),
        );
        this.#apiMods = this.params.mapmods.map((x) => {
            return { acronym: x };
        });
        ...
        interaction.options.getNumber("bpm")
            ? (this.params.overrideBpm = interaction.options.getNumber("bpm"))
            : null;
    ...
    async setParamsLink() {
        const messagenohttp = this.input.message.content
            .replace("https://", "")
            .replace("http://", "")
            .replace("www.", "");
        const tmod = this.setParamMods();
    ...
    async setParamMapGone(mapTemp: {
        ...
        if (!mapTemp.map && mapTemp.set) {
            ...
            if (
                !this.mapset?.beatmaps?.[0] ||
                this.mapset?.beatmaps?.length == 0
            ) {
    ...
    async execute() {
        ...
        const inputModalDiff = new Discord.StringSelectMenuBuilder()
            .setCustomId(
                `${helper.versions.releaseDate}-Select-map-${this.commanduser.id}-${this.input.id}-diff`,
            )
            .setPlaceholder("Select a difficulty");
        const inputModalSearch = new Discord.StringSelectMenuBuilder()
            .setCustomId(
                `${helper.versions.releaseDate}-Select-map-${this.commanduser.id}-${this.input.id}-search`,
            )
        ...
        if (this.params.showBg) {
            ...
            await this.execute_map(buttons, inputModalDiff, inputModalSearch);
        }
        ...
        if (this.params.isppCalc) {
            buttons.addComponents(
                new Discord.ButtonBuilder()
                    .setCustomId(
                        `${helper.versions.releaseDate}-Map-${this.name}-${this.commanduser.id}-${this.input.id}-${this.params.mapid}${this.params.mapmods && this.params.mapmods.length > 0 ? "+" + this.params.mapmods.join(",") : ""}`,
                    )
            ...
            buttons.addComponents(
                new Discord.ButtonBuilder()
                    .setCustomId(
                        `${helper.versions.releaseDate}-DetailDisable-${this.name}-${this.commanduser.id}-${this.input.id}`,
                    )
                    .setStyle(helper.buttons.type.current)
            ...
            buttons.addComponents(
                new Discord.ButtonBuilder()
                    .setCustomId(
                        `${helper.versions.releaseDate}-DetailEnable-${this.name}-${this.commanduser.id}-${this.input.id}`,
                    )
                    .setStyle(helper.buttons.type.current)
    ...
    protected checkNullParams() {
        if (!this.params.mapid && !this.params.query) {
            ...
            if (!this.params.mapmods || this.params.mapmods.length == 0) {
                this.params.mapmods = temp.mods.map(
                    (x) => x.acronym,
                ...
                if (!(
                    this.params.mapmods.includes("HR") ||
                    this.params.mapmods.includes("EZ")
                )) {
    ...
    protected async checkQueryType(
        inputModalSearch: Discord.StringSelectMenuBuilder,
    ) {
        if (this.params.query == null) {
            ...
            const mapidtest = await osuapi.v2.beatmaps.search({
            ...
            data.storeFile(
                mapidtest,
                this.params.query.replace(/[\W_]+/g, "").replaceAll(" ", "_"),
                "mapQuerydata",
            );
            ...
            if (mapidtest.beatmapsets.length == 0) {
                await this.sendError(
                    helper.errors.map.search_nf(this.params.query),
                );
                return;
            }
            try {
                ...
                for (
                    let i = 0;
                    i < mapidtest.beatmapsets[0].beatmaps.length;
                    i++
                ) {
                    if (
                        this.params.query.includes(
                            mapidtest.beatmapsets[0].beatmaps[i].version,
                        )
                    ) {
                ...
                mapidtest2 = mapidtest.beatmapsets[0].beatmaps.sort(
            ...
            for (let i = 0; i < mapidtest?.beatmapsets?.length && i < 25; i++) {
                ...
                if (!curmapset) break;
                const curmap = curmapset.beatmaps.sort(
                    (a, b) => b.difficulty_rating - a.difficulty_rating,
                )[0];
                        ...
                        .setEmoji(
                            `${
                                curmap.mode_int == 0
                                    ...
                                    : curmap.mode_int == 1
                                      ? helper.emojis.gamemodes.taiko
                                      : curmap.mode_int == 2
                                        ? helper.emojis.gamemodes.fruits
                                        : curmap.mode_int == 3
                                          ? helper.emojis.gamemodes.mania
                                          : helper.emojis.gamemodes.standard
                            }` as Discord.APIMessageComponentEmoji,
                        )
                        .setLabel(`${curmapset.title} // ${curmapset.creator}`)
                        .setDescription(
                            `[${curmap.version}] ${curmap.difficulty_rating}⭐`,
                        )
                        .setValue(`${curmap.id}`),
                );
            }
    ...
    protected setModalDifficulty(
        inputModalDiff: Discord.StringSelectMenuBuilder,
    ) {
        if (
            typeof this.mapset?.beatmaps == "undefined" ||
            this.mapset?.beatmaps?.length < 2
        ) {
            inputModalDiff.addOptions(
                new Discord.StringSelectMenuOptionBuilder()
                    ...
                    .setDescription(
                        `${calculate.fixLongDecimal(this.map.difficulty_rating)}⭐`,
                    )
                    .setValue(`${this.map.id}`),
            );
        } else {
            for (let i = 0; i < this.mapset.beatmaps.length && i < 25; i++) {
                const curmap = this.mapset.beatmaps
                    .slice()
                ...
                inputModalDiff.addOptions(
                    new Discord.StringSelectMenuOptionBuilder()
                        ...
                        .setDescription(
                            `${calculate.fixLongDecimal(curmap.difficulty_rating)}⭐`,
                        )
                        .setValue(`${curmap.id}`),
                );
            }
    ...
    protected async execute_map(
        ...
        try {
            ...
            totaldiff =
                ...
                ppComputed[0].difficulty.stars?.toFixed(2)
            ...
            if (
                (tstmods.includes("DT") || tstmods.includes("NC")) &&
                tstmods.includes("HT")
            ) {
        ...
        const embed = new Discord.EmbedBuilder()
            .setURL(
                `https://osu.ppy.sh/beatmapsets/${this.map.beatmapset_id}#${map.mode}/${this.map.id}`,
            )
            .setThumbnail(
                osuapi.other.beatmapImages(this.map.beatmapset_id).list2x,
            )
            .setTitle(maptitle);
        ...
        if (this.params.overwriteModal != null) {
            frmod = this.params
                .overwriteModal as Discord.StringSelectMenuBuilder;
        }

        if (!(inputModalDiff.options.length < 1)) {
            this.ctn.components.push(
                new Discord.ActionRowBuilder().addComponents(inputModalDiff),
            );
        }
        if (!(inputModalSearch.options.length < 1)) {
            this.ctn.components.push(
                new Discord.ActionRowBuilder().addComponents(frmod),
            );
        }
    ...
    protected checkMapMods() {
        if (this.params.mapmods == null) {
            ...
            this.params.mapmods = osumodcalc.mod.fix(
                this.params.mapmods,
                this.map.mode,
            );
            this.#apiMods = this.params.mapmods.map((x) => {
    ...
    protected converts(): [osuapi.types_v2.BeatmapExtended, boolean] {
        ...
        if (
            this.params.mode &&
            this.params.mode != this.map.mode &&
            this.params.mode != "osu"
        ) {
            for (const beatmap of this.mapset.converts) {
                if (
                    beatmap.mode == this.params.mode &&
                    beatmap.id == this.map.id
                ) {
                    map = beatmap;
                    successConvert = true;
                    break;
    ...
    protected async embedStart(
        map: osuapi.types_v2.BeatmapExtended,
        allvals,
        totaldiff: string,
        ppComputed: rosu.PerformanceAttributes[],
        buttons: Discord.ActionRowBuilder,
    ) {
        const mapname = formatters.parseUnicodeStrings(
            {
    ...
    protected async embedMap(
        ...
        const exMapDetails =
            this.mapCounts(this.map.beatmapset, map) +
        ...
        embed
            ...
            .addFields([
                ...
                {
                    name: "DOWNLOAD",
                    value: Object.entries(mirrors(this.mapset.id, this.map.id))
                ...
                }, // [osu!direct](osu://b/${this.map.id}) - discord doesn't support schemes other than http, https and discord
                {
                    name: "MAP DETAILS",
                    value:
                        `${this.statusEmoji(map)} | ${helper.emojis.gamemodes[map.mode]} | ${ppComputed[0].difficulty.maxCombo ?? this.map.max_combo}x combo \n ` +
                        `${this.params.detailed == 2 ? exMapDetails : ""}`,
        ...
        if (
            this.map?.owners &&
            !(
                this.map?.owners?.length == 1 &&
                this.map?.owners?.[0].id == this.mapset.user_id
            )
        ) {
        ...
        buttons.addComponents(
            new Discord.ButtonBuilder()
                .setCustomId(
                    `${helper.versions.releaseDate}-User-${this.name}-any-${this.input.id}-${this.map.user_id}+${this.map.mode}`,
                )
                .setStyle(helper.buttons.type.current)
        ...
        buttons.addComponents(
            new Discord.ButtonBuilder()
                .setCustomId(
                    `${helper.versions.releaseDate}-Leaderboard-${this.name}-${this.commanduser.id}-${this.input.id}`,
                )
                .setStyle(helper.buttons.type.current)
    ...
    async failGraph(map: osuapi.types_v2.BeatmapExtended) {
        ...
        const graph = new BarGraphBuilder({
            ...
            colours: [
                helper.colours.rainbowPastelRGB.red,
                helper.colours.rainbowPastelRGB.blue,
            ],
            settings: {},
        });
        const image = await graph.execute();
    ...
    protected async embedPerformance(
        embed: Discord.EmbedBuilder,
        map: osuapi.types_v2.BeatmapExtended,
        allvals,
        totaldiff: string,
