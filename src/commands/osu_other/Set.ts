...
export class Set extends OsuCommand {
    ...
    async setParamsMsg() {
        ...
        this.params.skin = this.setParam(
            this.params.skin,
            ["-skin"],
            "string",
    ...
    getOverrides(): void {
        if (this.input.overrides.type != null && this.input.type == "message") {
            switch (this.input.overrides.type) {
                case "mode":
                    [this.params.mode, this.params.name] = [
                        this.params.name as osuapi.types_v2.GameMode,
                        this.params.mode,
                    ];
                    break;
                case "skin":
                    [this.params.skin, this.params.name] = [
                        this.params.name,
                        this.params.skin,
                    ];
            }
    ...
    async execute() {
        ...
        const findname: helper.tooltypes.dbUser =
            (await helper.vars.userdata.findOne({
        ...
        if (findname == null) {
            try {
                await helper.vars.userdata.create({
                    ...
                    skin:
                        this.params.skin ??
                        "Default - https://osu.ppy.sh/community/forums/topics/129191?n=117",
                    location: "",
                    timezone: "",
                });
            ...
            const affectedRows = await helper.vars.userdata.update(updateRows, {
                where: { userid: this.commanduser.id },
            });
            ...
            if (affectedRows.length > 0 || affectedRows[0] > 0) {
                ...
                this.ctn.content =
                    "There was an error trying to update your settings";
                log.commandErr(
                    "Database error (update) ->" + affectedRows,
                    this.input.id,
                    "osuset",
                    this.input.message,
                    this.input.interaction,
