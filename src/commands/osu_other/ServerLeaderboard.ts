...
export class ServerLeaderboard extends OsuCommand {
    ...
    async setParamsBtn() {
        ...
        this.params.mode = this.input.message.embeds[0].footer.text.split(
            " | ",
        )[0] as osuapi.types_v2.GameMode;

        this.params.page = 0;
        if (this.input.buttonType == "BigLeftArrow") {
            this.params.page = 1;
        }
        const pageFinder = this.input.message.embeds[0].footer.text
            .split(" | ")[1]
            .split("Page ")[1];
    ...
    async execute() {
        ...
        if (typeof +this.params.id == "number") {
            const tempguild = helper.vars.client.guilds.cache.get(
                this.params.id,
            );
            if (tempguild) {
                const isThere = tempguild.members.cache.has(
                    this.commanduser.id,
                );
        ...
        let cache:
            ...
            | Discord.Collection<string, Discord.User>;
        ...
        const users = (await this.getUsers(this.params.mode)).filter((x) =>
            checkCache.includes(x.discord),
        );
        ...
        for (let i = 0; i < users.length && i < 10; i++) {
            ...
            if (!cur) break;
            const pad = i + 1 >= 10 ? (i + 1 >= 100 ? 3 : 4) : 5;
        ...
        serverlb.setFooter({
            text:
                this.params.mode +
                ` | Page ${this.params.page + 1}/${Math.ceil(users.length / 10)}`,
        });
