...
export class RankPP extends OsuCommand {
    ...
    async setParamsInteract() {
        ...
        this.params.mode =
            (interaction.options.getString(
                "mode",
            ) as osuapi.types_v2.GameMode) ?? "osu";
    }
    getOverrides(): void {
    ...
    async execute() {
        ...
        switch (this.params.get) {
            case "pp":
                {
                    returnval = await data.getRankPerformance(
                        "pp->rank",
                        this.params.value,
                        this.params.mode,
                    );
                    output =
                        "approx. rank #" +
                        calculate.separateNum(Math.ceil(returnval.value));
                    Embed.setTitle(
                        `Approximate rank for ${this.params.value}pp`,
                    );
                }
                break;
            case "rank":
                {
                    ...
                    output =
                        "approx. " +
                        calculate.separateNum(
                            calculate.fixLongDecimal(returnval.value),
                        ) +
                        "pp";

                    Embed.setTitle(
                        `Approximate performance for rank #${this.params.value}`,
                    );
                }
                break;
        }
