...
export class Gif extends Command {
    ...
    async execute() {
        ...
        if (gifSearch?.data?.results?.length > 1) {
            gifSelection = gifSearch?.data?.results?.map(
                (x) => x.media_formats.gif.url,
            );
        }
        ...
        const embed = new Discord.EmbedBuilder()
            .setTitle(
                baseString
                    .replace("user", this.commanduser.username)
                    .replace("target", this.params.target.username),
            )
            .setImage(
                gifSelection[Math.floor(Math.random() * gifSelection.length)],
            );
