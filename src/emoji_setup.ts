...
export async function setup() {
    const guild = helper.vars.client.guilds.cache.get(helper.vars.config.emojiGuild);
    if (!guild) {
        throw new Error('Could not find guild matching ID: ' + helper.vars.config.emojiGuild);
    }
    let basePath = helper.path.precomp + '/files/emojis/';
    const ems = guild.emojis;
    for (const [emoji, path] of Object.entries(emojis) as [keyof typeof emojis, string][]) {
...
function emojiExists(name: string, cache: Discord.Collection<string, Discord.GuildEmoji>) {
...
function emojiGet(name: string, cache: Discord.Collection<string, Discord.GuildEmoji>) {
    let uname = name.toLowerCase();
    for (const [idx, emoji] of cache) {
...
const buttonkeys = ['page_first', 'page_previous', 'page_search', 'page_next', 'page_last',
    'refresh',
    'details_default', 'details_more', 'details_less',
    'random',
    'graph',
    'map',
    'user',
...
const emojikeys = [
    ...
    'bpm', 'total_length',
    'modeosu', 'modetaiko', 'modefruits', 'modemania',
    'ranked', 'approved', 'loved', 'graveyard',
    'Ranking_XH', 'Ranking_X', 'Ranking_SH', 'Ranking_S', 'Ranking_A', 'Ranking_B', 'Ranking_C', 'Ranking_D', 'Ranking_F',
    'osu_online', 'osu_offline',
