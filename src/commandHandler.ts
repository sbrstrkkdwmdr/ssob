...
export class CommandHandler extends InputHandler {
    async onMessage(message: Discord.Message) {
        ...
        if (this.validateMessage(message)) {
            const args = message.content
                .slice(helper.vars.config.prefix.length)
                .trim()
                .split(/ +/g);
    ...
    validateMessage(message: Discord.Message) {
        if (!message.content.startsWith(helper.vars.config.prefix))
            return false;
        return true;
    }
    ...
    commandCheck(
        ...
        const requireReactions: string[] = ["poll", "vote"];
        const requireMsgManage: string[] = ["purge"];
        ...
        if (
            requireEmbedCommands.includes(cmd) &&
            !checks.botHasPerms(message ?? interaction, ["EmbedLinks"])
        ) {
            missingPermsBot.push("EmbedLinks");
        }
        if (
            requireReactions.includes(cmd) &&
            !checks.botHasPerms(message ?? interaction, ["AddReactions"])
        ) {
            missingPermsBot.push("AddReactions");
        }
        if (
            requireMsgManage.includes(cmd) &&
            !checks.botHasPerms(message ?? interaction, ["ManageMessages"])
        ) {
            missingPermsBot.push("ManageMessages");
        }
        if (
            botRequireAdmin.includes(cmd) &&
            !checks.botHasPerms(message ?? interaction, ["Administrator"])
        ) {
            missingPermsBot.push("Administrator");
        }
        if (
            userRequireAdminOrOwner.includes(cmd) &&
            !(
                checks.isAdmin(
        ...
        if (
            userRequireOwner.includes(cmd) &&
            !checks.isOwner(message?.author?.id ?? interaction.member.user.id)
        ) {
            missingPermsUser.push("Owner");
        }

        if (
            missingPermsBot.length > 0 &&
            !(message ?? interaction).channel.isDMBased()
        ) {
        ...
        if (disabled.includes(cmd)) {
            commandTools.sendMessage({
                ...
                args: {
                    content:
                        "That command is currently disabled and cannot be used.",
                },
                canReply,
            });
            return false;
        ...
        if (
            ["hug", "kiss", "lick", "pet", "punch", "slap"].includes(cmd) &&
            helper.vars.config.tenorKey == "INVALID_ID"
        ) {
            commandTools.sendMessage({
                type: "message",
                message,
                ...
                args: {
                    content:
                        "gif commands cannot be currently used (error: unset tenor key)",
                },
                canReply,
            });
            return false;
    ...
    commandSelect(cmd: string, args: string[]) {
        let tnum: string;
        if (
            scorelist.some((x) => cmd.startsWith(x)) &&
            !scorelist.some((x) => cmd == x)
        ) {
        ...
        if (
            [
                ...
                "deps",
                "source",
            ].some((x) => x.toLowerCase() == cmd.toLowerCase())
        ) {
            args = [cmd];
            cmd = "info";
        }
        ...
        switch (cmd) {
            ...
            case "badges":
                ...
                break;
            case "bws":
            case "badgeweightsystem":
            case "badgeweight":
            case "badgeweightseed":
            case "badgerank":
