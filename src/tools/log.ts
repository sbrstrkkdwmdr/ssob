...
export function stdout(message?: any, ...optionalParams: any[]) {
    const text = optionalParams
        ? util.format(message)
        : util.format(message, optionalParams ?? null);
...
Channel ID:   ${message?.channelId ?? interaction?.channelId}`;
    if (opts.length > 0) {
        ...
        const padding = opts
            ...
            .sort((a, b) => b - a)[0];
        opts.forEach((opt) => {
            txt += `\n${(opt.name + ":").padEnd(padding, " ")}${util.format(opt.value)}`;
        });
    }
    txt += "\n====================================================";
    out(
        txt,
        `${helper.path.logs}/${(message?.guildId ?? interaction?.guildId) ? "cmd/" + (message?.guildId ?? interaction?.guildId) + ".log" : "commands.log"}`,
        true,
    );
}

export function commandErr(
    input: string,
    id: string | number,
    name: string,
    message: Discord.Message<any>,
