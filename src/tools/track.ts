...
export async function editTrackUser(fr: {
    ...
    if (!fr.action || fr.action == "add") {
        try {
            ...
            const previous = await helper.vars.trackDb.findOne({
                where: { osuid: fr.userid },
            });
            const prevchannels: string[] =
                previous?.dataValues?.guilds?.split(",") ?? [];
        ...
        const curuser = await helper.vars.trackDb.findOne({
            where: { osuid: fr.userid },
        });
        const curguilds: string[] =
            curuser.dataValues[`guilds${fr.mode}`].split(",");
...
export async function trackUser(fr: {
    user: string;
    mode: string;
    inital?: boolean;
}) {
    if (!fr.user) return;
    const curdata: osuapi.types_v2.Score[] & osuapi.types_v2.Error =
        await osuapi.v2.scores.best({
            user_id: +fr.user,
    ...
    if (curdata?.[0]?.user_id && fr.inital == true) {
        fs.writeFileSync(
            `${helper.path.main}/trackingFiles/${curdata[0].user_id}_${fr.mode}.json`,
    ...
    if (curdata?.[0]?.user_id) {
        ...
        fs.writeFileSync(
            `${helper.path.main}/trackingFiles/${curdata[0].user_id}_${fr.mode}.json`,
            JSON.stringify(
                curdata.slice().sort((a, b) => b.pp - a.pp),
                null,
                2,
            ),
        );
...
export async function getEmbed(data: {
    ...
    const embed = new Discord.EmbedBuilder()
        .setTitle(
            `${data.scoredata.beatmapset.title} [${data.scoredata.beatmap.version}]`,
        )
        .setURL(
            `https://osu.ppy.sh/beatmapsets/${data.scoredata.beatmapset.id}#osu/${data.scoredata.beatmap.id}`,
        )
        .setAuthor({
        ...
        .setThumbnail(
            `${data.scoredata?.user?.avatar_url ?? helper.defaults.images.user.url}`,
        )
...
export async function trackUsers(totalTime: number) {
    const allUsers: helper.tooltypes.trackUser[] =
        (await helper.vars.trackDb.findAll()) as any;
    for (let i = 0; i < allUsers.length; i++) {
        ...
        setTimeout(
            () => {
                ...
                let willFetch = false;
                if (!(
                    typeof user.osuid == "undefined" ||
                    user.osuid == null ||
                    user.osuid == undefined
                )) {
                    if (
                        `${user.guildsosu}`.length > 0 &&
                        `${user.guildsosu}`.length != 4
                    ) {
                        trackUser({
                            user: user.osuid,
                            mode: "osu",
                    ...
                    if (
                        `${user.guildstaiko}`.length > 0 &&
                        `${user.guildstaiko}`.length != 4
                    ) {
                        trackUser({
                            user: user.osuid,
                            mode: "taiko",
                    ...
                    if (
                        `${user.guildsfruits}`.length > 0 &&
                        `${user.guildsfruits}`.length != 4
                    ) {
                        trackUser({
                            user: user.osuid,
                            mode: "fruits",
                ...
                if (willFetch == true) {
                    ...
                    log.stdout(
                        `Tracking cancelled - User ${user.osuid} has no tracked channels`,
                    );
                }
            },
...
export async function sendMsg(embed: Discord.EmbedBuilder, curuser: string) {
    const userobj: helper.tooltypes.trackUser =
        (await helper.vars.trackDb.findOne({
            where: {
                osuid: curuser,
            },
        })) as any;
    const guilds = userobj?.guilds?.includes(",")
        ? userobj?.guilds?.split(",")
        : [userobj?.guilds];
        ...
        helper.vars.client.guilds.cache.forEach(async (guild2) => {
            if (guilds.includes(guild2.id)) {
                ...
                if (curset?.dataValues?.trackChannel) {
                    ...
                    log.stdout(
                        `Found channel in guild settings - ${curset?.dataValues.trackChannel}`,
                    );
                } else {
                    log.stdout(
                        "Found channel in guild settings - No channel set",
                    );
                }
            }
        });
    ...
    channels = await channels.filter(
        (item, index) => channels.indexOf(item) === index,
    );

    setTimeout(() => {
        channels
            .filter((item, index) => channels.indexOf(item) === index)
            .forEach((channel) => {
                const curchannel: Discord.GuildTextBasedChannel =
                    helper.vars.client.channels.cache.get(
                        channel,
