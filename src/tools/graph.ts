...
export abstract class GraphBuilder {
    ...
    protected generateCanvas() {
        ...
        // @ts-expect-error CanvasRenderingContext2D not assignable to ChartItem
        // see https://www.chartjs.org/docs/latest/api/#chartitem
...
export class LineGraphBuilder extends GraphBuilder {
    ...
    constructor({
        ...
        colours,
        showDataLabels,
    }: GraphBuilderInput<LineGraphSettings>) {
        super({
            x,
            y,
            dataLabels,
    ...
    protected validateSettings() {
        ...
        if (this.settings.isFlipped && this.settings.fill) {
            this.settings.fill = false;
            log.stdout(
                "Chart Error: Fill cannot be enabled while isFlipped is true",
            );
        }
    }
...
export class BarGraphBuilder extends GraphBuilder {
    ...
    constructor({
        ...
        colours,
        showDataLabels,
    }: GraphBuilderInput<BarGraphSettings>) {
        super({
            x,
            y,
            dataLabels,
...
export class OldGraphBuilder {
    ...
    constructor({ x, y, label, other, extra = [] }: graphInput) {
        this.x = x.map((foo: string | number) =>
            typeof foo == "string" ? foo : foo + "",
        );
        this.y = y;
        this.label = label;
        this.other = other;
    ...
    checkSettings() {
        if (
            this.other.startzero == null ||
            typeof this.other.startzero == "undefined"
        ) {
        ...
        if (
            this.other.displayLegend == null ||
            this.other.displayLegend == undefined ||
            typeof this.other.displayLegend == "undefined"
        ) {
            this.other.displayLegend = false;
        }
        if (
            this.other.type == null ||
            this.other.type == undefined ||
            typeof this.other.displayLegend == "undefined"
        ) {
    ...
    extraDataItem(
        ...
        if (data?.data?.length > 0) {
            ...
            const newclr = colourcalc.hsvToRgb(
                nHSV.h + diff * index,
                nHSV.s,
                nHSV.v,
            );
            ...
            if (
                this.other?.type == "bar" &&
                this.other?.stacked == true &&
                this.other?.stacksSeparate == true
            ) {
                data.customStack
    ...
    protected get config_old() {
        ...
        if (this.other?.type == "bar") {
            cfgopts["elements"] = {
                ...
                borderColor: this.other?.barOutline
                    ? "rgb(255, 255, 255)"
                    : (this.other.lineColour ?? "rgb(101, 101, 135)"),
                borderWidth: 2,
            };
        }
