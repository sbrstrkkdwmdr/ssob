...
export async function updateUserStats(
    ...
    try {
        findname = allUsers.find(
            (u: any) =>
                (u as helper.tooltypes.dbUser).osuname.toLowerCase() ==
    ...
    if (findname) {
        switch (mode) {
            ...
            default:
                {
                    await helper.vars.userdata.update(
                        {
                            osupp: !isNaN(user?.statistics?.pp)
                                ? user?.statistics?.pp
                                : 0,
                            osurank: !isNaN(user?.statistics?.global_rank)
                                ? user?.statistics?.global_rank
                                : 0,
                            osuacc: !isNaN(user?.statistics?.hit_accuracy)
            ...
            case "taiko":
                await helper.vars.userdata.update(
                    {
                        taikopp: !isNaN(user?.statistics?.pp)
                            ? user?.statistics?.pp
                            : 0,
                        taikorank: !isNaN(user?.statistics?.global_rank)
                            ? user?.statistics?.global_rank
                            : 0,
                        taikoacc: !isNaN(user?.statistics?.hit_accuracy)
                            ? user?.statistics?.hit_accuracy
                            : 0,
                    },
                    {
            ...
            case "fruits":
                await helper.vars.userdata.update(
                    {
                        fruitspp: !isNaN(user?.statistics?.pp)
                            ? user?.statistics?.pp
                            : 0,
                        fruitsrank: !isNaN(user?.statistics?.global_rank)
                            ? user?.statistics?.global_rank
                            : 0,
                        fruitsacc: !isNaN(user?.statistics?.hit_accuracy)
                            ? user?.statistics?.hit_accuracy
                            : 0,
                    },
                    {
            ...
            case "mania":
                await helper.vars.userdata.update(
                    {
                        maniapp: !isNaN(user?.statistics?.pp)
                            ? user?.statistics?.pp
                            : 0,
                        maniarank: !isNaN(user?.statistics?.global_rank)
                            ? user?.statistics?.global_rank
                            : 0,
                        maniaacc: !isNaN(user?.statistics?.hit_accuracy)
                            ? user?.statistics?.hit_accuracy
                            : 0,
                    },
                    {
...
export async function searchUser(searchid: string, findMode: boolean) {
    const findname = await helper.vars.userdata.findOne({
        where: { userid: searchid ?? "0" },
    });
    let user;
    let errorValue;
    let mode;
...
export async function searchUserFull(searchid: string, userdata: any) {
    const findname = await userdata.findOne({
        where: { userid: searchid ?? "0" },
    });
    let user;
    let mode;
        ...
        if (name.includes("osudata")) {
            if (
                ...
                return JSON.parse(
                    fs.readFileSync(
                        path +
                            `${name.toLowerCase()}${id}_${mode ?? "osu"}.json`,
                        "utf-8",
                    ),
                );
            }
            ...
            if (fs.existsSync(path + `${name.toLowerCase()}Ranked${id}.json`)) {
                return JSON.parse(
                    fs.readFileSync(
                        path + `${name.toLowerCase()}Ranked${id}.json`,
                        "utf-8",
                    ),
                );
            } else if (
                fs.existsSync(path + `${name.toLowerCase()}Loved${id}.json`)
            ) {
                return JSON.parse(
                    fs.readFileSync(
                        path + `${name.toLowerCase()}Loved${id}.json`,
                        "utf-8",
                    ),
                );
            } else if (
                fs.existsSync(path + `${name.toLowerCase()}Approved${id}.json`)
            ) {
                return JSON.parse(
                    fs.readFileSync(
                        path + `${name.toLowerCase()}Approved${id}.json`,
                        "utf-8",
                    ),
                );
            } else if (
                fs.existsSync(path + `${name.toLowerCase()}Graveyard${id}.json`)
            ) {
                return JSON.parse(
                    fs.readFileSync(
                        path + `${name.toLowerCase()}Graveyard${id}.json`,
                        "utf-8",
                    ),
                );
            }
            ...
            if (
                fs.existsSync(path + `${name.toLowerCase()}${id}_${type}.json`)
            ) {
                return JSON.parse(
                    fs.readFileSync(
                        path + `${name.toLowerCase()}${id}_${type}.json`,
...
export function getPreviousId(
    ...
    try {
        const init = JSON.parse(
            fs.readFileSync(
                `${helper.path.cache}/previous/${type}${serverId}.json`,
                "utf-8",
            ),
        ) as {
            id: string | false;
...
export function writePreviousId(
    type: "map" | "user" | "score",
    serverId: string,
    data: {
        id: string;
    ...
    data["default"] = false;
    data["last_access"] = moment.default
        .utc()
...
export function debug(
    ...
    try {
        fs.writeFileSync(
            `${helper.path.cache}/debug/${name}/${pars}_${serverId}.json`,
            JSON.stringify(data, null, 2),
        );
    } catch (error) {
...
export function recommendMap(
    baseRating: number,
    retrieve: "closest" | "random",
    mode: osuapi.types_v2.GameMode,
    maxRange?: number,
) {
    ...
    if (sorted.length == 0) {
        ...
        const filter = sorted.filter(
            (x) =>
                x?.difficulty_rating > baseRating - maxRange &&
                x?.difficulty_rating < baseRating + maxRange,
        );
        if (filter.length > 0) {
...
export async function userStatsCache(
    user: osuapi.types_v2.UserStatistics[] | osuapi.types_v2.User[],
    mode: osuapi.types_v2.GameMode,
    type: "Stat" | "User",
) {
    switch (type) {
                ...
                for (let i = 0; i < user.length; i++) {
                    ...
                    if (typeof findname == "undefined" || !findname) {
                        await helper.vars.statsCache.create({
                            ...
                            [mode + "rank"]:
                                `${curuser?.statistics?.global_rank ?? NaN}`,
                            [mode + "acc"]:
                                `${curuser?.statistics?.hit_accuracy ?? NaN}`,
                        });
                    } else {
                        await helper.vars.statsCache.update(
                            {
                                [mode + "pp"]:
                                    `${curuser?.statistics?.pp ?? NaN}`,
                                [mode + "rank"]:
                                    `${curuser?.statistics?.global_rank ?? NaN}`,
                                [mode + "acc"]:
                                    `${curuser?.statistics?.hit_accuracy ?? NaN}`,
...
export async function getRankPerformance(
    type: "pp->rank" | "rank->pp",
    value: number,
    mode: osuapi.types_v2.GameMode,
): Promise<{
    value: number;
    ...
    for (let i = 0; i < users.length; i++) {
        ...
        if (!(
            isNaN(+curuser[`${mode ?? "osu"}pp`]) ||
            typeof curuser[`${mode ?? "osu"}pp`] == "undefined" ||
            !curuser[`${mode ?? "osu"}pp`] ||
            isNaN(+curuser[`${mode ?? "osu"}rank`]) ||
            typeof curuser[`${mode ?? "osu"}rank`] == "undefined" ||
            !curuser[`${mode ?? "osu"}rank`]
            ...
            case "pp->rank":
                {
                    //pp within 10
                    return (
                        (calculate.isWithinPercentage(
                            +x.pp,
                            +x.pp * 0.000001,
                            +value,
                        ) ||
                            calculate.isWithinValue(+x.pp, 1, +value)) ??
                        false
                    );
                }
                break;
            case "rank->pp":
                {
                    //rank within 1%
                    return (
                        calculate.isWithinPercentage(+x.rank, 1, +value) ??
                        false
                    );
                }
                break;
    ...
    if (tempChecking.length > 0) {
        ...
        switch (type) {
            case "pp->rank":
                {
                    return {
                        value: rankData.median ?? rankData.mean,
                        isEstimated: false,
                    };
                }
                break;
            case "rank->pp":
                {
                    return {
                        value: ppData.median ?? ppData.mean,
                        isEstimated: false,
                    };
                }
                break;
        ...
        case "pp->rank":
            {
                ...
                const line = stats.linearRegressionLine(
