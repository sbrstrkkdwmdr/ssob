...
export async function recentActivity(i: { user_id: number; limit?: number }) {
...
export async function kudosu(i: {
    ...
    return (await helper.requests.get_v2(url, params)) as Promise<
        apitypes.KudosuHistory[]
    >;
}

export async function lookup(i: { checksum?: string; id?: number }) {
    if (!(i.checksum && i.id))
        throw new Error("Please input a checksum or ID to lookup");
    if (!helper.allowed("public")) throw new Error("Missing scope: public");
...
export async function scores(i: {
    ...
    const params = helper.setParams(i, { limit: 100 }, [
        "limit",
        "offset",
        "mode",
        "include_fails",
 ...
 * past 24h
 */
export async function scoresRecent(
    i: scoreListParams & { include_fails: 1 | 0 },
) {
    return await scores({
        user_id: i.user_id,
        mode: i.mode,
