...
export async function scores(i: {
    ...
    return (await helper.requests.get_v2(url, params)) as Promise<
        apitypes.BeatmapScores<apitypes.Score>
    >;
}
...
export async function userScore(i: {
    ...
    return (await helper.requests.get_v2(
        url,
        params,
    )) as Promise<apitypes.BeatmapUserScore>;
}
...
export async function map(i: { id: number }) {
    if (!i.id) throw new Error("Missing beatmap ID");
    if (!helper.allowed("public")) throw new Error("Missing scope: public");
...
export async function maps(i: { ids: number[] }) {
...
export async function mapLookup(i: {
    ...
    if (!(i.filename || i.checksum || i.id))
        throw new Error("Please input a filename, checksum or ID to lookup");
    if (!helper.allowed("public")) throw new Error("Missing scope: public");
...
export async function mapset(i: { id: number }) {
...
export async function search(i: { query: string; cursor_string?: string }) {
    if (!i.query) throw new Error("Please input a query");
    if (!helper.allowed("public")) throw new Error("Missing scope: public");

    const url = `/beatmapsets/search`;

    const params = helper.setParams(i, { q: encodeURIComponent(i.query) }, [
        "cursor_string",
    ]);
...
export async function packs(i: { type?: string; cursor_string?: string }) {
    if (!helper.allowed("public")) throw new Error("Missing scope: public");
    ...
    return (await helper.requests.get_v2(url, params)) as Promise<
        apitypes.BeatmapPack[]
    >;
}

export async function pack(i: { pack: string; legacy_only: 1 | 0 }) {
    if (!helper.allowed("public")) throw new Error("Missing scope: public");
...
// https://github.com/cyperdark/osu-api-extended/blob/master/api/v2/beatmaps_events_list.ts
export async function events(i: {}) {
