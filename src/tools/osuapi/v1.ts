...
/**
 * convert these keys to these values when sending via the API
 */
const convert: Dict = {
    set_id: "s",
    map_id: "b",
...
export async function beatmaps(i: {
    ...
    return (await helper.requests.get_v1(url, params)) as Promise<
        apitypes.Beatmap[]
    >;
}
...
export async function match(i: { match_id: number }) {
...
export async function replay(i: {
    ...
    if (!(i.user && i.map_id && i.score_id))
        throw new Error(
            "Missing user, map ID and score ID. Please use either a score ID or map ID + user",
        );
