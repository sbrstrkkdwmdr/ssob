...
export function checkCredentials(version: 1 | 2): [boolean, string] {
    if (version == 1 && !helper.credentials.key) {
        return [
            false,
            "Missing API key. Please use add a key with v1.login(key)",
        ];
    } else if (version == 2) {
        let msgs = "";
        let p = true;
        if (!helper.credentials.id) {
            msgs +=
                "Missing client ID. Please input the ID with v2.login(id,secret)\n";
            p = false;
        }
        if (!helper.credentials.secret) {
            msgs +=
                "Missing client secret. Please input the secret with v2.login(id,secret)\n";
            p = false;
        }
...
export async function get_v2(url: string, params: Dict, tries: number = 0) {
    if (tries > 3) {
        throw new Error(
            "Exceeded try count. Please ensure credentials are valid",
        );
    }
...
export async function post_v2(
    ...
    const data = (
        await axios
            .post(inp.toString(), JSON.stringify(body), {
                headers: {
                    Authorization: `Bearer ${helper.credentials?.auth?.access_token}`,
                    "Content-Type": "application/json",
                    Accept: "application/json",
...
export async function get_v1(url: string, params: Dict, tries: number = 0) {
    ...
    helper.log(
        "v1 get: " + inp.toString().replace(helper.credentials.key, "KEY"),
    );
    const data = (
        await axios
            .get(inp.toString(), {
