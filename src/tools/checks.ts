    ...
    for (const key in keys) {
        ...
        if (keys[key][2] == "object") {
            if (cfg.hasOwnProperty(key)) {
                ...
                console.log(
                    `Property ${key} is either missing or invalid. Setting to default value of ${format(keys[key][3])}`,
                );
                cfg[key] = keys[key][3];
            }
            ...
            if (!cfg.hasOwnProperty(key)) {
                if (keys[key][0])
                    throw new Error(
                        `Property ${key} is missing. Bot cannot run without this property`,
                    );
                console.log(
                    `Property ${key} is either missing or invalid. Setting to default value of ${format(keys[key][3])}`,
                );
            ...
            typeof cfg[key] != keys[key][2]
        ) {
            if (keys[key][0])
                throw new Error(
                    `Property ${key} is invalid. Make sure key is set to type ${format(keys[key][2])}`,
                );
            console.log(
                `Property ${key} is either missing or invalid. Setting to default value of ${format(keys[key][3])}`,
            );
...
export function isAdmin(userid: string | number, guildid: string | number) {
    try {
        if (helper.vars.client.guilds.cache.has(`${guildid}`)) {
            ...
            const curmem = curguild?.members?.cache?.has(`${userid}`)
                ? curguild?.members?.cache?.get(`${userid}`)
                : null;
            if (curmem != null) {
 ...
 */
export function botHasPerms(
    object: Discord.Interaction | Discord.Message,
    requiredPerms: Discord.PermissionsString[],
) {
    ...
    const channel = helper.vars.client.channels.cache.get(object.channelId) as
        Discord.TextChannel | Discord.ThreadChannel;
    const botchannelperms = channel
        .permissionsFor(helper.vars.client.user.id)
        .toArray();
...
function getcfg() {
    try {
        const p = JSON.parse(
            fs.readFileSync(path.precomp + "/config/config.json", "utf-8"),
        );
        return p;
    } catch (err) {
        return {
